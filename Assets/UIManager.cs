...
public class UIManager : MonoBehaviour
{
    ...
    //switch ui state
    public void SwitchView(PlayView view)
    {
        ...
        GardenManager.instance.staticGridBoundsMap.GetComponent<Animator>().SetBool("show", view == PlayView.SIDE);
        GardenManager.instance.scrubsMap.GetComponent<Animator>().SetBool("show", view != PlayView.SIDE);
        CameraManager.instance.cameraAnimator.SetBool("showSide", view == PlayView.SIDE);

        //ui & tutorial visibility
        //initialPhaseTutorial.GetComponent<Animator>().SetBool("show", view == PlayView.SIDE);
        sideBarAnimator.SetBool("show", view == PlayView.SIDE);
        stickerActionsPanel.gameObject.SetActive(view != PlayView.RESULTS);
        turnButton.gameObject.SetActive(view != PlayView.RESULTS);
        resultsBar.gameObject.SetActive(view == PlayView.RESULTS);

        sideButton.image.sprite = view == PlayView.SIDE ? sideButtonEnableSprite : sideButtonNormalSprite;
        turnButton.image.sprite = view == PlayView.SIDE ? turnButtonEnableSprite : turnButtonNormalSprite;
        //sideButton.image.sprite = view == PlayView.SIDE ? sideButtonEnableSprite : sideButtonNormalSprite;
